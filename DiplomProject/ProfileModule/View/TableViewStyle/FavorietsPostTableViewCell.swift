...
//  DiplomProject
//
//  Created by Ekaterina Saveleva on 27.10.2023.
//
...
class FavorietsPostTableViewCell: UITableViewCell {
    ...
    func setup(post: NSManagedObject) {
        ...
        self.imageInPosts.image = UIImage(named: post.value(forKey: "image") as? String ?? "")
    ...
    private func layout() {
        [nameAuthor,  postViewsCount, descriptionOfPosts, postLikesCount, likesInPosts, imageInPosts, tapLikeButton, postViews].forEach({contentView.addSubview($0)})
        //tapLikeButton,
